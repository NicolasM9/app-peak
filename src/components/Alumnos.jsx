  ...
  async function load() {
    ...
    const [{ data: al }, { data: pl }, { data: pg }] = await Promise.all([
      supabase
        .from('alumnos')
        .select(
          'id, nombre, telefono, email, fecha_nacimiento, deporte, estado, plan_id, ajuste_motivo, ajuste_monto, modalidad_rutina, medicion_nutricional, paga_directo_profe, estado_fisico, lesion_detalle, lesion_desde, objetivos, franja, planes(nombre, precio_mensual)',
        )
        .order('nombre'),
